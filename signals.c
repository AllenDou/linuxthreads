...
test_again:
  ...
  for (s = 1; s <= NSIG; s++) {
    if (sigismember(set, s) == 1 && s != PTHREAD_SIG_CANCEL) {
      sigdelset(&mask, s);
...
/* In POSIX threads, raise(sig) must be equivalent to
   pthread_kill(pthread_self(), sig) instead of the usual
   kill(getpid(), sig).
   Unfortunately, raise is not defined as a weak alias in libc 5,
   so the code below is commented out.
   This is not too serious (yet), since with the current non-conforming
   implementation of signals in LinuxThreads, 
   pthread_kill(pthread_self(), sig) behaves exactly like
   kill(getpid(), sig). */
#if 0
int raise (int sig)
{
